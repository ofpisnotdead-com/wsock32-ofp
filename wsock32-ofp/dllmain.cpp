...
extern "C" hostent * __stdcall __E__43__(char* name)
{
	typedef hostent * (__stdcall* pS)(char*);
	pS pps = (pS)p[43];

	const char* gamespy = "master.gamespy.com";
	const char* ofpisnotdead = "master.ofpisnotdead.com";
	hostent * rv;

	if (strcmp(name, gamespy) == 0) {
		rv = pps((char*)ofpisnotdead);
	}
	else {
		rv = pps(name);
	}
